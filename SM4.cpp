...
DWORD T(DWORD x, E_Encrypt_Mode is_encrypt, E_Demo_Mode is_demo_mode) {
	...
	if (is_demo_mode)
		make_demonstration_note(L"\t1) S-box:\n");
	
	for (BYTE n = 0, j = 0; n <= 24 && j < 4; n += 8, j++) {
		q[j] = (BYTE)((x >> n) & 0xFF);
		
		if (is_demo_mode)
			make_demonstration_note(L"\t\'%02X\' = \'%02X\'\n", q[j], SBox[q[j]]);

		q[j] = SBox[q[j]];
		tau ^= q[j] << n;
	}
	if (is_demo_mode)
		make_demonstration_note(L"\tS-box result: Tau = %08X\n", tau);

	// 2. L(tau)
	if (is_encrypt == EEM_Encrypt) {
		if (is_demo_mode)
			make_demonstration_note(L"\t2) L(tau): Tau xor (Tau <<< 2) xor (Tau <<< 10) xor (Tau <<< 18) xor (Tau <<< 24) = \n"
		...
		if (is_demo_mode)
			make_demonstration_note(L"\t2) L(tau): Tau xor (Tau <<< 13) xor (Tau <<< 23) = %08X xor (%08X <<< 13) xor (%08X <<< 23) = %08X\n",
...
void Key_Expansion(DWORD MK[], DWORD rk[], E_Demo_Mode is_demo_mode) {
	if (is_demo_mode)
		make_demonstration_note(L"===============================================================Key Expansion===============================================================\n"
								"MK = %08X %08X %08X %08X;\n", MK[0], MK[1], MK[2], MK[3]);
